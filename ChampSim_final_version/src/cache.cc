...
set<int> very_cold,very_hot,cold,hot;
int get_complement(int set1)
{
  return 2047 - set1;
}
long long sev = 512;
...
uint32_t CACHE::get_set(uint64_t address)
{
    ...
    if(cache_type != IS_LLC) return (uint32_t) (address & ((1 << lg2(NUM_SET)) - 1));
    ...
    calls++;
    int set1 =  (uint32_t) (address & ((1 << lg2(NUM_SET)) - 1)); 
    freq[set1]++;
    if(calls%sev==0)
    {
      sev*=2;
      ...
      for(int i=0;i<512;i++)
      {
        very_cold.insert(v[i].second);
      }
      for(int i=512;i<1024;i++)
      {
        cold.insert(v[i].second);
      }
      for(int i=1024;i<1536;i++)
      {
        hot.insert(v[i].second);
      }
      for(int i=1536;i<2048;i++)
      {
        very_hot.insert(v[i].second);
      }
    ...
    if(get_way(address,set1)!=NUM_WAY || very_cold.count(set1) || cold.count(set1))
    {
      return set1;
    }
    else if(hot.count(set1))
    {
      for(auto it:cold)
      {
        if(get_way(address,it)!=NUM_WAY) return it;
      }

      for(int i=0;i<16;i++)
      {
        if(!block[set1][i].valid) return set1;
      }
      ...
      int temp = cold.size();
      temp = calls%temp;
      set<int>::iterator it = cold.begin();
      ...
      {
        buffer.insert(block[(*it)][calls%16].tag);
        block[(*it)][calls%16].tag = address;
        return (*it);
      }
      return(*(it));
    }
    else
    {
      ...
      int temp = very_cold.size();
      temp = calls%temp;
      set<int>::iterator it = very_cold.begin();
      for(int i=0;i<temp;i++)
      {
        it++;
      }

...
map<string, pair<string, string> > readBTB(const string& filename) {
    ifstream file(filename);
    map<string, pair<string, string> > BTB;
    ...
    while (getline(file, line) && total < 1e6) {
        ...
        if (i_type[0] == 'b') {
            ...
            if (!file.eof()) {
                ...
                if (BTB[program_counter].first != target_address) {
                    BTB[program_counter].first = target_address;
                    if (target_address != nextcore_pc) {
                        correct += 1;
                        BTB[program_counter].second = "00";
                        BHT[program_counter].push_back({"NT", "NT"});
                    }else{
                        incorrect += 1;
                        BTB[program_counter].second = "01";
                        BHT[program_counter].push_back({"NT", "T"});
                    }
                }else {
                    if (target_address != nextcore_pc && BTB[program_counter].second == "00") {
                        correct += 1;
                        BHT[program_counter].push_back({"NT", "NT"});
                    }else if (target_address == nextcore_pc && BTB[program_counter].second == "00") {
                        incorrect += 1;
                        BHT[program_counter].push_back({"NT", "T"});
                        BTB[program_counter].second = "01";
                    }else if (target_address != nextcore_pc && BTB[program_counter].second == "01") {
                        correct += 1;
                        BHT[program_counter].push_back({"NT", "NT"});
                        BTB[program_counter].second = "00";
                    }else if (target_address == nextcore_pc && BTB[program_counter].second == "01") {
                        incorrect += 1;
                        BHT[program_counter].push_back({"NT", "T"});
                        BTB[program_counter].second = "11";
                    }else if (target_address != nextcore_pc && BTB[program_counter].second == "11") {
                        incorrect += 1;
                        BHT[program_counter].push_back({"NT", "T"});
                        BTB[program_counter].second = "10";
                    }else if (target_address == nextcore_pc && BTB[program_counter].second == "11") {
                        correct += 1;
                        BHT[program_counter].push_back({"T", "T"});
                    }else if (target_address != nextcore_pc && BTB[program_counter].second == "10") {
                        incorrect += 1;
...
int main(){
    ...
    map<string, pair<string, string> > BTB = readBTB(filename);
    ...
    ofstream outputFile;
    outputFile.open("2_bit_predictor_TABLES.txt");
    ...
    for(auto it: BTB) {
        outputFile << it.first << "  " << it.second.first << endl;
    }
    outputFile << endl;

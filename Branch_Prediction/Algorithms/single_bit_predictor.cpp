...
map<string, pair<string, string> > readBranchTargetBuffer(const string& filename) {
            ...
            if (!file.eof()) {
                ...
                if (BTB[program_counter].first != target_address) {
                    ...
                    if (target_address != nextcore_pc) {
                        correct += 1;
                        BTB[program_counter].second = "not_taken";
                        BHT[program_counter].push_back({"NT", "NT"});
                    }else{
                        incorrect += 1;
                        BTB[program_counter].second = "taken";
                        BHT[program_counter].push_back({"NT", "T"});
                    }
                }else {
                    if (target_address != nextcore_pc && BTB[program_counter].second == "not_taken") {
                        correct += 1;
                        BHT[program_counter].push_back({"NT", "NT"});
                    }else if (target_address != nextcore_pc && BTB[program_counter].second == "taken") {
                        incorrect += 1;
                        BHT[program_counter].push_back({"T", "NT"});
                        BTB[program_counter].second = "taken";
                    }else if (target_address == nextcore_pc && BTB[program_counter].second == "not_taken") {
                        incorrect += 1;
                        BHT[program_counter].push_back({"NT", "T"});
                        BTB[program_counter].second = "not_taken";
                    }else {
...
int main(){
    ...
    map<string, pair<string, string> > BTB = readBranchTargetBuffer(filename);
    ...
    ofstream outputFile;
    outputFile.open("single_bit_predictor_TABLES.txt");
